...
const showToast=(msg:string)=>{setToastMsg(msg);setTimeout(()=>setToastMsg(''),3500);};const copyEmail=(e?:React.MouseEvent)=>{if(e)e.preventDefault();navigator.clipboard.writeText('zephyrxzep@gmail.com');showToast(t('toast_copied'));};const handleStartProject=()=>{setIsWarping(true);showToast('🕳️ WARP DRIVE: SINGULARITY ENTRY ENGAGED');setTimeout(()=>{setContactOpen(true);setIsWarping(false);},420);};const handleSubmit=async(e:React.FormEvent)=>{e.preventDefault();setSentStatus(true);const accessKey=process.env.NEXT_PUBLIC_WEB3FORMS_KEY;if(!accessKey){showToast('❌ ไม่สามารถส่งข้อความได้ กรุณาติดต่อทางอีเมลโดยตรง');setSentStatus(false);return;}try{const res=await fetch('https://api.web3forms.com/submit',{method:'POST',headers:{'Content-Type':'application/json',Accept:'application/json'},body:JSON.stringify({access_key:accessKey,name:formState.name,email:formState.email,message:formState.message,subject:`✦ Portfolio Project Inquiry from ${formState.name}`,from_name:formState.name})});const data=await res.json();if(data.success||res.ok){showToast('✓ ส่งข้อความเข้าอีเมลเรียบร้อยแล้ว!');setFormState({name:'',email:'',message:''});}else{showToast('❌ เกิดข้อผิดพลาดในการส่งข้อความ');}}catch{showToast('❌ ไม่สามารถส่งข้อความได้ กรุณาตรวจสอบอินเทอร์เน็ต');}finally{setSentStatus(false);setContactOpen(false);}};useEffect(()=>{let guardUntil=0,raf=0,lastActive='',lastSaveY=-999;const apply=()=>{raf=0;let current='origin';for(const[id]of sections){const e=document.getElementById(id);if(e&&e.getBoundingClientRect().top<innerHeight*.48)current=id}if(current!==lastActive){lastActive=current;setActive(current);history.replaceState(null,'','#'+current)}if(Date.now()>guardUntil&&Math.abs(scrollY-lastSaveY)>48){lastSaveY=scrollY;try{sessionStorage.setItem('home:y',String(Math.round(scrollY)))}catch{}}};const scroll=()=>{if(!raf)raf=requestAnimationFrame(apply)};try{const y=+(sessionStorage.getItem('home:y')||0);const ts=+(sessionStorage.getItem('home:restore')||0);if(y>0&&(Date.now()-ts<8000||document.referrer.includes('/work/'))){guardUntil=Date.now()+600;const r=()=>scrollTo({top:y,behavior:'auto'});r();requestAnimationFrame(r);setTimeout(r,60);setTimeout(r,200);setTimeout(r,400)}sessionStorage.removeItem('home:restore')}catch{}apply();addEventListener('scroll',scroll,{passive:true});const heroEl=document.getElementById('origin');const hio=heroEl?new IntersectionObserver(([e])=>document.body.classList.toggle('hero-out',!e.isIntersecting),{rootMargin:'80px'}):null;if(heroEl&&hio)hio.observe(heroEl);return()=>{removeEventListener('scroll',scroll);if(raf)cancelAnimationFrame(raf);hio?.disconnect();document.body.classList.remove('hero-out')}},[]);return <main>
